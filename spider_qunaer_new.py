...
ip_pool = redis.ConnectionPool(host='localhost', port=6379, db=int(0), max_connections=1000)
client = redis.Redis(connection_pool=ip_pool)
flights_data = {
    "result": [],
    "need": 0
}
                ...
                if resp['status'] != -1:
                    if resp.get('code') != -1 or resp.get('code') is None and resp.get("['result']['flightPrices']") != {}: # 假数据
                        ...
                        if resp.get('code') != -1 or resp.get('code') is None and resp.get(
                            ...
                            return {'error': f"{pro_ip}出现故障，二次仍无法请求成功"}
                else:
                    return {'error': f"航班数据为空"}
            else:
